<?php
	...
	$code_invite_tickets_admin = $code_worker2 = '';
	if(PRAVA_1){
			...
			foreach($row_worker as $i => $m){
					...
					$code_worker2 .= $t->HtmlInviteWorkers(array(	'id' 		=> $m['id'],
																'flag' 		=> $m['flag'],
																'prava_id' 	=> $m['prava_id'],
																'avatar' 	=> $m['avatar'],
																'name' 		=> $m['company'],
			...
			$code_invite_tickets_admin = '<section class="workers" id="workers">
								<div class="container">
									<div class="workers-title">
										Админы тикетов
									</div>
									<div class="workers-wrapper">
										...
										<button id="button_miw" class="workers-new blue-button modal_invite_workers">
											+ Пригласить сотрудника для тикетов
										</button>
									</div>
								</div>
							</section>';

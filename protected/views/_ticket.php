<?php
	...
	$kol 	= !empty($member['ticket']['kol'])?$member['ticket']['kol']:'';
 	...
 	$tr 	= $trM = $replyMessage = $headData = $need_link = $navPanel = $tfo = $select_wraper_tfo = $img_file = '';
...
*/	
	if(!empty($rowt)){
					...
					//var_dump($ticket_flag);
						...
						if (!in_array(LOGIN_ID, $adminsArr)) {
							...
							if ($ticket_flag == 1) {
									...
									$tfo = '<option value="1">В ошибку</option>';
								} else {
									$tfo = '';
								}
		...
		foreach($rowt as $i => $t){
			...
			$name_company = !empty($t["name_rcmc"])?$t["name_rcmc"]:'';
	...
	if($tid){
		...
		*/
		foreach($rtf as $key => $p){
			...
			$sl_video .= '<div class="carousel-item '.$active2.'"> <video class="img-fluid" autoplay loop muted><source src="/files/tickets/1585/'.$p['file_name'].'" type="video/mp4"> </video> </div>'; 

$(function(){
	$("body").append('<div class="modal fade" id="vmodal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true"></div>'+
	...
	$("body").on("click", ".modal_search", function(){
		var d = $(this).data();
			...
			$.post("/modal_search", {mclass:d.mclass,flag_buy_sell:d.flag_buy_sell}, 
				function(data){
					if(data.code){
						modal.html(data.code);
	...
	*/
	...
	$("body").on("click", ".action_subscriptions", function(){
		...
		var d = obj.data();
		var where = d.where
			$.post("/action_subscriptions", { id:d.id , where:where }, 
				function(data){
					if(where=='company_profile'){
							...
							if (data.ok) {
								...
								if(data.flag_modal_amo_accounts){
									ModalAmoAccount(data.qrq_id);
								}
							}else{
								webix.message({type:"error", text:data.code});
							}
	...
	$("body").on("click", ".cron_amo_search", function(){
		...
		var d = $(this).data();
		var values = {};
			$("#vmodal_refresh .checkbox_qrq_article_id_"+d.buy_sell_id+"").each(function(indx, element){
				if ($(element).prop('checked')){
					var dd = $(element).data();
		...
		var im = $(".input-message").val(),
			d = $(this).data();
				...
				//console.log(d.mid);
 				...
 				$.post("/reply_message", { messagetext:im, mid:d.mid }, 
					function(data){
						if(data.ok){
							console.log(data);							
...
// Модаль авторизации на стороних ресурсах AMO
function ModalAmoAccount(qrq_id){
		$('#vmodal').removeData();
		var d = $(this).data();
			$.post("/modal_amo_accounts", { id:qrq_id }, 
				function(data){
					if(data.code){
						$('#vmodal').html(data.code);
						$('#vmodal').modal();
						$('#vmodal').on('shown.bs.modal',
								function(e){
										$('#amo_accounts-form').bootstrapValidator('destroy');
										$('#amo_accounts-form').bootstrapValidator({
											feedbackIcons: {
												valid: 'glyphicon glyphicon-ok',
												invalid: 'glyphicon glyphicon-remove',
												validating: 'glyphicon glyphicon-refresh'
		...
		$('#my_company_id_1c-form').bootstrapValidator({
			...
			var bv = $form.data('bootstrapValidator');
				$.post("/save_my_company_id_1c", $form.serialize(),
					function(data){
							if(data.ok){
								onReload('');//надо обновить define параметры
								//webix.message("Сохранено");
							}else{
								webix.message({type:"error", text:data.code});
							}
...
// модальное окно вопрос со сторонних серверов QRQ
function ModalQrqBuySell(){
		$('#vmodal_refresh').removeData();
			$.post("/modal_qrq_buy_sell", {}, 
				function(data){
					if(data.code){
						$('#vmodal_refresh').html(data.code);
						$('#vmodal_refresh').modal();
						$('#vmodal_refresh').on('shown.bs.modal',
								function(e){
										//
								}
						).on('hidden.bs.modal', function (e) {
...
function SaveSearchFilterParamCompany( where , categories_id , search_value , flag , flag_reload ){
		...
		var flag_buy_sell 	= $('#nav-first #flag_buy_sell').val();
		if(categories_id>0){
			// оставляем переданную категорию
			flag_search = true;// выбрана категория в autocomplete_search - перезагружаем страницу с текущими параматрами
		}else{
		...
		*/
		$.post("/save_search_filter_param_company", {	where:			where,
			...
			function(data){
				//
				if(flag_search||flag_reload){
					SearchBuySell( search_value );
				}
			}
		);
...
// поиска
function SearchBuySell(value){
$.post("/search_buy_sell", { value:value , where:window.location.pathname }, 
					function(data){
						onReload(data.url);
					}
			);
...
// Обработка покупки со стороннего сервиса
function AmoBasket( json , amount , buy_sell_id , where ){
	

	$.post("/amo_basket", { json:json , amount:amount , buy_sell_id:buy_sell_id , where:where }, 
		function(data){

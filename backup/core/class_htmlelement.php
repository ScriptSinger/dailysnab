...
class HtmlElement {
	...
	function SelectAttributeValue( $p=array() ){
		$code = $opt = '';
		$r = reqAttributeValue(array('flag'=>'group_attribute_value_id','categories_id'=>$p['categories_id'],'attribute_id'=>$p['attribute_id']));
		$arr_av = explode(',',$r['attribute_value_id']);
	...
	function SelectWhoCompany( $p=array() ){
		...
		$code = '
				<select id="who_company" name="who_company" multiple="multiple" class="form-control select2">
							'.$opt.'
				</select>
				';
	...
	function ButtonSubscriptions( $p=array() ){
		$in = fieldIn($p, array('flag','company_id_out'));

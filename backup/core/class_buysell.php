...
class HtmlBuySell extends HtmlServive 
{
	...
	function ProverkaActionBuySell( $p=array() ){
		...
		$r = isset($p['row'])? $p['row'] : reqBuySell(array('id'=>$in['buy_sell_id']));
	...
	function ActionBuySell( $p=array() ){
		...
		//if( $in['url_priznak']=='buy' && $in['flag_buy_sell']==2 && ($in['status_buy_sell_id']==2||$in['status_buy_sell_id']==3) ){
				if( $r_min['flag_offer_min_cost'] || $r_min['flag_offer_min_cost2'] ){// есть предложение
		...
		*/
		...
		if( $ok || $r_min['flag_offer_min_cost'] || $r_min['flag_offer_min_cost2'] ){		
	...
	function UpdateStatusBuySell( $p=array() ){
		...
		if($arr['ok']){
				...
				//		if($arr["buy_sell"]["categories_id"]=='821'){
				...
				if( ($in['status']==1||$in['status']==2||$in['status']==3||$in['status']==4) && $flag_limit ){
						...
						$r = reqBuySell(array('company_id' => COMPANY_ID , 'id' => $in['buy_sell_id']));
				...
				elseif( $in['status']==13 ){
						...
						if($row_bs['status_buy_sell_id']==11){
							...
							if($row_bs['copy_id']){// отмена покупки от предложения к заявки (при покупки объявления copy_id не создается)
								...
								$r_n1 = reqBuySell(array('id'=>$row_bs['copy_id']));// заявка родитель
						...
						elseif($row_bs['status_buy_sell_id']==12){
							...
							$r_n1 = reqBuySell(array('id'=>$row_bs['parent_id']));// родитель
	...
	function ChangeStatusBuy( $p=array() ){
		$ok = false;
		$in = fieldIn($p, array('buy_sell_id','status','where_status','data_insert','amount','copy_id'));
	...
	function CopyRowBuySell( $p=array() ){
			...
			$login_id 		= ($in['login_id'])? 						$in['login_id'] 		: $row_bs['login_id'];
			...
			if($in['status']==6){// копия заявки
				...
				$url = 'buy';
				$r = reqBuySell(array('id'=>$row_bs['parent_id']));
	...
	function NomenclatureInsertUpdate( $p=array() ){
		...
		$ok = $STH = $nomenclature_id = $categories_id = false;	
		...
		if(!$in['id']){// СОЗДАНИЕ
					...
					$arr = reqInsertNomenclature(array(	
														'name'					=> $in['name'],
														'categories_id'			=> $in['categories_id'],
														'reqInsertNomenclature'	=> $in['1c_nomenclature_id']
														));
					if($arr['STH']){
	...
	function SaveCacheBuySell( $p=array() ){
		...
		if($in['flag_buy_sell']==2){// формируем строку "Мои заявки"

			$tr_2 = $t->TrMyBuy(array('m'=>$row,'cache'=>true,'flag_cache'=>$in['flag_cache']));
			
		}
		
		if( $in['flag_buy_sell']==1 || ( $in['flag_buy_sell']==2 && ($row['status_buy_sell_id']>=10) ) ){// формируем строку "Мои объявления" и Активы
		...
		$row_bs = $p['row_bs'];
			...
			// в случае объявления flag_buy_sell=2
				...
				if($in['where']=='modal_offer11'){// получаем строку для обновления , при покупке
						...
						$row_p = reqBuySell(array('id'=>$row['parent_id']));
						if($row_p['status_buy_sell_id']==5){
							$flag_clear_parent = true;
						}
				}
	
		return array( 'ok'=>$ok , 'code'=>$code , 'flag_clear_parent'=>$flag_clear_parent , 'parent_id'=>$row_bs['parent_id'] );
	}

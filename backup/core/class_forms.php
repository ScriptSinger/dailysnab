...
*/			
	...
	function FormMyCompany( $p=array() ){
		...
		$bottom =  $this->Input(
			...
			<button type="submit" class="button-blue" >Старт</button>
		</div>';
	...
	function FormBuySell( $p=array() ){
		...
		$st_dn = $save_button = $count_status_buysell = $availability = $select_categories = $span_clear_form = $st_span_select_categories = $required_name = $required_file = '';
		...
		if( !$in['id'] ){
			...
			$r 			= array('id'=>0,'name'=>'','categories_id'=>0,'comments'=>'','urgency_id'=>5,'form_payment_id'=>'',
								...
								'stock_id'=>'','company_id2'=>'','id_1c'=>'' /*,'unit'=>'','amount'=>''*/);
		...
		if(!$in['id']){
			...
			elseif($in['flag_buy_sell']==5){
				...
				$save_button = '	<button type="submit" data-status="1" class="btn btn-outline-primary"><span data-feather="save"></span></button>
								'.$button23.'';
			}
		}else{
		...
		$code_stock = $code_company_id2 = $code_id_1c = '';
		if( $in['flag_buy_sell']==2 || $in['flag_buy_sell']==5 ){
	...
	function FormBuyAmount( $p=array() ){
		...
		$row	= reqBuySell(array('id' => $in['buy_sell_id']));
												...
												function(data){
														...
														if(data.ok){
															...
															if(data.code_qrq){// покупка со стороннего ресурса, форму предлагает
																$("#form_buy-form'.$in['buy_sell_id'].'").after(data.code_qrq);
	...
	function FormNomenclature( $p=array() ){
		...
		$content ='
			<div class="app-form-wrapper btn-option">
				...
				<div class="col-sm-12">
					<div class="form-group">
						'.$this->Select(	array(	'id'		=> '1c_nomenclature_id',
												'class'		=> 'form-control select2',
												'value'		=> $r['1c_nomenclature_id'],
												'data'		=> array('placeholder'=>'1С')
											),
										array(	'func'		=> 'req1cNomenclature',
												'param'		=> array('flag' => 'bind','id'=>$r['1c_nomenclature_id']),
	...
	// Авторизации на стороних ресурсах AMO
	function FormAmoAccount( $p=array() ){
  ...
  <div class="card">
    ...
    <div id="collapsePro" class="collapse" aria-labelledby="headingPro" data-parent="#accordion" data-type_skills="1">
      <div class="card-body">
		...
		<p>Стоимость подписки Pro в составляет 399 рублей в месяц, </p>
  ...
  <div class="card">
    ...
    <div id="collapseVip" class="collapse" aria-labelledby="headingVip" data-parent="#accordion" data-type_skills="2">
      <div class="card-body">
		...
		<p>Стоимость подписки Vip в составляет 51000 рублей в месяц, </p>
...
$(document).ready(function(){
  $(".collapse").on(\'show.bs.collapse\', function(e){
	...
	function GetCodeForm( $p=array() ){
		...
		$in = fieldIn($p, array('id','phone_email_code'));  //, code_type
		...
		<div class="form-wrapper2"><p class="form-title">Введите код подтверждения</p>
				<div class="form-group">
				...
				<div class="">
					В течении двух минут вы получите сообщение с кодом подтверждения
					<br />
					<small>
						<span id="again_link_text" class="show">Получить новый код можно через <span id="timer"></span></span>
						<span id="again_link" class="hide">Получить код еще раз</span>
					</small>
				</div></div>';
	...
	function FormCancelPodpiska( $p=array() ){
		...
		$top 	= '<div class="form-wrapper2">
				<div class="modal-body__head">
					<div class="modal__title"><h3>Отменить подписку?</h3></div>							
				</div>';		
		...
		$content = '
			...
			<div class="modal-body__content">
				...
				<div class="row">
					...
					<div class="col-sm-2">
						<img src="/image/profile-icon.png" alt="Лого" id="message-logo" class="user-company-logotype" onclick="$$(\'upload_logo_message\').fileDialog();">
							<!--
							<div class="profile-img">
									<div id="div_upload_profile" style="display:none;">
										<button id="upload_result_profile" class="btn btn-success btn-sm upload-result" data-id="" 
																														data-div_id="upload_demo_profile" 
																														data-img_id="img_avatar_profile"><span data-feather="check">Сохранить изображение</span></button>
									</div>
									<img src="" id="img_avatar_profile" alt="" class="rounded-circle img_avatar" height="40" data-file_id="upload_avatar_profile">

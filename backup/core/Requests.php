...
//vecho($sql);	
	...
	function reqAttributeValue($p=array()) {
		$sql = '';
		$sql_select = 'av.id, av.categories_id, av.attribute_id, av.attribute_value_id, 
					sav.attribute_value';
		$arr = array();
		$one = false;
		...
		if($in['flag']=='group_attribute_value_id'){
			$sql_select = " GROUP_CONCAT(CONCAT(av.attribute_value_id)) attribute_value_id ";
	...
	function reqCompany($p=array()) {
		...
		$arr = array();
		$one = false;
		$in = fieldIn($p, array('id','login_id','phone','one','not_login_id','who1','who2',
								'kol','flag','ipage','count_page','flag_account','value','company_id_md5','id_1c'));
		...
		$sql_select = " 	c.id, c.login_id, c.flag_account, c.legal_entity_id, c.company, c.email, c.iti_phone, c.phone, c.cities_id,
						c.position, c.tax_system_id, c.who1, c.who2, c.pro_mode, c.id_1c,
	...
	function reqCompanyAutocomplete($p=array()) {
		...
		$sql = " SELECT c.id, c.flag_account, c.company, c.email, sle.legal_entity
				...
				WHERE c.legal_entity_id=sle.id ".$sql." 
	...
	function reqCompanyAdmin($p=array()) {
		...
		$sql = "	SELECT c.id, c.login_id, c.flag_account, c.company, c.who1, c.who2, c.pro_mode,
	...
	function reqBuySell($p=array()) {
		...
		*/
		...
		$sql = "	SELECT 	bs.id, bs.parent_id, bs.copy_id, bs.login_id login_id_bs, bs.company_id, bs.company_id2, bs.flag_buy_sell, bs.name, bs.url, 
				...
				".$sql_inner_join."
				WHERE bs.company_id=c.id AND cities.id=bs.cities_id AND bs.categories_id=sc.id AND bs.urgency_id=su.id AND sbs.id=bs.status_buy_sell_id AND sc.unit_id=sunit.id 
						".$sql_offer."
						".$sql."
				ORDER BY ".$sql_order." ";
/*
...
*/
	...
	function reqMyBuySellCache($p=array()) {
		$sql = $sql_flag_buy_sell = $sql_categories_id = $sql_cities_id = '';
		$arr = array();
		$one = false;
		$in = fieldIn($p, array('id','company_id','flag_buy_sell','status_buy_sell_id','active',
								...
								'login_id_interests', 'company_id_interests' ));
		...
		$sql = "	SELECT bs.id, bs.login_id, bs.flag_buy_sell, bs.status_buy_sell_id, bs.categories_id, bs.responsible_id, 
				...
				FROM slov_status_buy_sell sbs, buy_sell_cache bsc, buy_sell bs
				LEFT JOIN company c2 ON c2.id=bs.company_id2
				WHERE bsc.buy_sell_id=bs.id AND sbs.id=bs.status_buy_sell_id 
						".$sql."
				ORDER BY CASE WHEN bs.data_status_buy_sell_23>bs.data_insert THEN bs.data_status_buy_sell_23 
																			ELSE bs.data_insert END DESC 
...
*/
	...
	function reqBuySell_pageBuy($p=array()) {
		...
		$sql = "	SELECT 	bs.id, bs.company_id, bs.name, bs.url,
...
*/		
	...
	function reqBuySell_PageSell($p=array()) {
		...
		$one = false;
		$arr = array();
		$in = fieldIn($p, array('val_grouping','categories_id','cities_id','value','flag_search','company_id','share_url'));
		
		$sql_order = " bs.data_insert DESC ";
		
		$sql_sell = " AND ( 
							bs.status_buy_sell_id IN (3) OR ( bs.status_buy_sell_id IN (2) AND bs.company_id IN ( 	SELECT t.company_id_out 
																		FROM subscriptions t 
																		WHERE t.company_id_in=".COMPANY_ID." ) 
											) 
						) ";
	...
	*/
	...
	function reqBuySellAttributeSixParam($p=array()) {
		...
		$sql_select = "	sa.attribute, 
						CASE WHEN bsa.attribute_value_id=0 THEN bsa.value ELSE avs.attribute_value END  attribute_value ";
		...
		if($in['flag']=='six'){
			...
			$sql_select = "	GROUP_CONCAT(DISTINCT sa.attribute ORDER BY ca.sort SEPARATOR ', ') attribute,
							GROUP_CONCAT(CASE WHEN bsa.attribute_value_id=0 THEN bsa.value ELSE avs.attribute_value END  ORDER BY ca.sort SEPARATOR ', ') attribute_value ";
			$sql_group = " ca.sort ";
		}
		
		$sql = "	SELECT ca.sort, sa.id, ".$sql_select."
				FROM slov_attribute sa, categories_attribute ca, buy_sell_attribute bsa
				...
				WHERE bsa.buy_sell_id=".$in['buy_sell_id']." AND ca.categories_id=".$in['categories_id']." AND sa.id=ca.attribute_id AND bsa.attribute_id=ca.attribute_id
	...
	function reqProverkaAmountBuy($p=array()) {
		...
		if($in['flag']==11){
			...
			$sql = "	SELECT 	CASE WHEN 
								(	SELECT SUM(bs.amount) FROM buy_sell bs
									WHERE bs.active=1 AND bs.status_buy_sell_id=11 AND
											bs.copy_id IN (
															SELECT bs2.id FROM buy_sell bs2
															WHERE bs2.id IN ( SELECT t.id FROM buy_sell t 
																			WHERE t.active=1 AND t.parent_id=".$in['buy_sell_id']." AND t.status_buy_sell_id=6 )
															)
								)
								>=
...
//vecho($sql);
	...
	function reqHistoryBuySell($p=array()) {
		...
		$sql = "	SELECT h.id, h.buy_sell_id, h.value_new, h.value_old, h.comments, 
	...
	function reqSearchRequestAutocomplete($p=array()) {
		...
		$sql = "	SELECT *
				FROM (
					...
					WHERE sc.active=1 /*AND sc.level=3*/ ".$sql2."
	...
	*/
	...
	function reqSearchFilterParamCompany($p=array()){
		...
		$sql = "	SELECT f.flag_buy_sell, f.categories_id, f.cities_id, f.flag, f.flag_interest, f.cookie_session
	...
	// Insert - Добавить html представление вопросов со стороних сервисов для ПОИСКА
	function reqInsertQrqHtmlQuestionSearch($p=array()) {

		$STH = PreExecSQL(" INSERT INTO qrq_html_question_search (session_id,html_question,qrq_id) VALUES (?,?,?); " ,
														array( session_id(),$p['html_question'],$p['qrq_id'] ));
		if($STH){
			$ok = true;
		}
	...
	// html представления вопросов со стороних сервисов (qrq) для ПОИСКА
	function reqQrqHtmlQuestionSearch($p=array()){
		$sql = '';
		//$in = fieldIn($p, array('value'));
		$arr = array();
		
		
		$sql = "	SELECT qq.id, qq.session_id, qq.qrq_id, qq.html_question
				FROM qrq_html_question_search qq
				WHERE qq.session_id=?
				ORDER BY qq.id ";

		$row = PreExecSQL_all($sql,array(session_id()));
	...
	function reqNavTabsStock($p=array()) {
		...
		$sql = "	SELECT 31 sbs_id, s.id stock_id, s.stock, IFNULL(bs.kol,0) kol
				FROM stock s
				LEFT JOIN (	SELECT bs.stock_id, COUNT(bs.id) kol
							FROM buy_sell bs
							WHERE bs.active=1 AND bs.flag_buy_sell=5 AND bs.status_buy_sell_id=31
							GROUP BY bs.stock_id ) bs ON bs.stock_id=s.id
...
*/
	...
	function reqSlovUnitVBind1c($p=array()){
		...
		$sql = "	SELECT 1 flag, t.name, t.measure_id, su.unit, su.okei
				...
				WHERE NOT su.id IN (	SELECT su.id
							FROM tmp_1c_slov_unit t, slov_unit su
	...
	function req1cTypenom($p=array()) {
		...
		$sql = "	SELECT tn.id, tn.company_id, tn.id_1c, tn.name
				FROM 1c_typenom tn
	...
	function req1cStock($p=array()) {
		...
		$sql = "	SELECT s.id, s.company_id, s.id_1c, s.name
				FROM 1c_stock s
	...
	function req1cCompany($p=array()) {
		...
		$arr = array();
		$one = false;
		$in = fieldIn($p, array('id_1c','flag','id'));
		...
		$sql = "	SELECT c.id, c.company_id, c.id_1c, c.name
				FROM 1c_company c
	...
	function req1cNomenclature($p=array()) {
		...
		$sql = "	SELECT n.id, n.company_id, n.id_1c, n.article, n.name, n.measure_id, n.typenom_id_1c
	...
	function reqAmoAccounts($p=array()) {
		...
		$arr = array();
		$one = false;
		$in = fieldIn($p, array('id','company_id','vendorid','flag','accounts_id','qrq_id'));
		...
		$sql = "	SELECT a.id, a.company_id, a.vendorid, a.login, a.password, a.accounts_id, a.accounts_title,
					sq.id qrq_id, sq.qrq, sq.login_id, sq.company_id company_id_qrq,
					c.company
	...
	function reqSlovQrq($p=array()) {
		...
		$arr = array();
		$one = false;
		$in = fieldIn($p, array('id','login_id'));
		...
		if($in['login_id']){
			$sql	= ' and sq.login_id=?';
			$arr = array($in['login_id']);
			$one = true;
		}

		$sql = "	SELECT sq.id, sq.qrq, sq.login_id, sq.company_id, sq.vendorid
				FROM slov_qrq sq
				WHERE 1=1 ".$sql." ";
...
/* 				if($in['categories_id']){
				...
				if($in['value']){
						$sql .= " AND ( LOWER(c.company) LIKE ? OR LOWER(tf.folder_name) LIKE ? OR LOWER(t.ticket_exp) LIKE ? )";
						array_push($arr , '%'.$in['value'].'%');
						array_push($arr , '%'.$in['value'].'%');
						array_push($arr , '%'.$in['value'].'%');
				}
		...
		$sql = "	SELECT t.id, t.folder_id, t.company_id, tf.companies_id, t.companies, t.ticket_exp, t.ticket_status, t.attachments, 
					...
					ORDER BY t.data_insert ASC LIMIT ".$start_limit." , 25
				";
...
// вывод данных о папках сообщений
	function reqChatFolders($p=array()) {
		...
		$arr = array();
		$one = false;
		$in = fieldIn($p, array('id','folder_name','owner_id','avatar','needs_id','status','companies_id','not_status'));

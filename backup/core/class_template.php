...
class HtmlTemplate extends HtmlServive 
{
			...
			$code = '<div class="request-head">
						...
						<ul class="request-menu">
							<li class="request-menu-item">
								<button>
									<a href="/buy-sell/'.$url_priznak.'" class="'.$active0.'">'.$str1.' '.$kol.'</a>
								</button>
							</li>
							'.$li.'
						</ul>
	...
	function CategoriesAttributeBuySell( $p=array() ){
		...
		if( ($in['flag_buy_sell']==2) && PRO_MODE && ($in['flag']=='change_categories') ){// предзаполненные поля, Значения, которые предзаполняются, берутся из предыдущей заявки.
				...
				$rbs 	= ($rb['id'])? reqBuySell(array('id' => $rb['id'])) : '';
		...
		*/
		...
		foreach($row as $i=>$m){
			...
			if($m[ $pole1 ]==1){// Цифровое поле
				...
				$elem = $this->HtmlTypeAttribute2(array('value'=>$m['attribute_value_id'],'attribute_id'=>$m['attribute_id'],'attribute'=>$m['attribute']));
	...
	function CategoriesAttributeAction( $p=array() ){
		...
		$row_bs = reqBuySell(array('id'=>$in['parent_id']));// данные заявки
		
		if($in['status']==10){// обнуляем для предложения
				$row_bs['cost'] = '';					$row_bs['currency_id'] = '';
				$row_bs['availability'] = '';			$row_bs['name'] = '';
				$row_bs['comments'] = '';				$row_bs['nomenclature_id'] = '';
				$row_bs['multiplicity'] = '';			$row_bs['min_party'] = '';
	...
	function TrModalOffer( $p=array() ){
			...
			$m = $p['row'];
				...
				$buy_offer = $amount_buy_current = '';
				$row_buy_offer = reqBuySell(array('parent_id'		=> $m['id'],
...
*/
	...
	function ModalSearch( $p=array() ){
		...
		if(!empty($rf)){
				...
				$active_flag_buy_sell1 = $active_flag_buy_sell2 = '';
				if($rf['flag_buy_sell']==1){
						$active_flag_buy_sell1 		= 'active';
						$str_active_flag_buy_sell	= 'Объявления';
						$flag_buy_sell				= 1;
				}else{
						$active_flag_buy_sell2 		= 'active';
						$str_active_flag_buy_sell	= 'Заявки';
						$flag_buy_sell				= 2;
				}
		...
		$content ='<div class="btn search-main">
				...
				<div class="hidden-wrapper">
					<ul class="nav nav-tabs setting-nav" id="myTab" role="tablist">
						<li class="nav-item">
							<a class="nav-link active first-link choosen" id="home-tab" data-toggle="tab" href="#nav-first" role="tab" aria-controls="home" aria-selected="true">
							  <img src="/image/icon/add.png" alt="" class="setting-img"><span>'.$str_active_flag_buy_sell.'</span>
							  <div class="cancel-choose">x</div>
							</a>
						</li>
						<li class="nav-item">
	...
	function TrPageSell( $p=array() ){
			...
			// кнопка купить на кого подписан
			$button_buy = '';
			if( $m['flag_subscriptions_company_in']||$m['flag_subscriptions_company_out'] ){
			...
			if( $m['kol_grouping'] && !$in['view_grouping'] ){
					$button_buy = '';
					$m['cities_name'] = $m['urgency'] = $m['data_status_buy_sell_23'] = $m['categories'] = '';//$day_noactive = 
					$m['comments'] = $m['form_payment'] = $m['cities_name'] = $m['unit'] = $m['amount'] = '';
					$button_grouping = $this->Input(	array(	'type'			=> 'button',
														'id'			=> 'view_grouping'.$m['id'],
														'class'			=> 'change-btn view_grouping',
						...
						<div class="sell-item__info">
							...
							<div class="sell-item_bottom">
								...
								<div class="sell-item_bottom-right">
									<span class="sell-item_bottom-right__cat">'.$m['categories'].'</span>
									<!--<span class="sell-item_bottom-right__icon">●</span>-->
									<span class="sell-item_bottom-right__name"><a href="/company-profile/'.$m['company_id'].'" target="_blank">'.$m['company'].'</a></span>
								</div>
							</div>
		...
		$r_dop = reqMyBuySell_DopParam(array('buy_sell_id'			=> $m['id'],
			...
			$code_shleyf1 = '';
				if($m['status_buy_sell_id']==1||$m['status_buy_sell_id']==2||$m['status_buy_sell_id']==3){
					...
					$code_shleyf1 = '';
					if($m['parent_id']){
						$r1 = reqBuySell(array( 'id' => $m['parent_id'] ));
						
						$r2 = (isset($r1['parent_id'])&&!empty($r1['parent_id']))? reqBuySell(array( 'parent_id' => $r1['parent_id'] , 'status_buy_sell_id' => 7 , 'flag'=>'shleyf_one' )) : array('id'=>'');
						if($r2['id']){
							$arr2 = $this->TrMyBuySellShleif(array('buy_sell_id'=>$r2['id'] , 'i'=>1));
							$code_shleyf1 = $arr2['tr'];
						}else{
							if(isset($r1['parent_id'])&&!empty($r1['parent_id'])){
								$r3 = reqBuySell(array( 'id' => $r1['parent_id'] ));
								if($r3['flag_buy_sell']==1){// куплено объявление
									/*
									$arr3 = $this->TrMyBuySellShleif(array('buy_sell_id'=>$r1['id'] , 'i'=>1));
									$code_shleyf1 = $arr3['tr'];
									*/
									$m['copy_id'] = $r1['id'];
								}
							}
			...
			// Не опубликованные, Опубликованные, Активные
	...
	function TrPageBuy( $p=array() ){
		...
		$row_np 		= isset($p['row_np'])? $p['row_np'] : array('notification_param_id_10'=>'','notification_param_id_11'=>'');
			...
			// Дать предложение
				...
				if( ($m['company_id']<>COMPANY_ID) || $row_share['share_url'] ){
						$form_buy_sell = '	
									<div class="btn-group">
										'.$this->Input(	array(	'type'			=> 'button',
																'id'			=> 'button_form_buy_sell'.$m['id'],
									...
									<div class="request-stages">
										<div class="request-stage">
											<div class="request-data-name 666">
												<p>
													<a href="/' . $m['url_cities'] . '/' . $m['url_categories'] . '/' . $m['url'] . '" target="_blank">
														' . $m['name'] . '
														<span class="bttip" style="display: none;">' . $m['name'] . '</span>
													</a>
												</p>
	...
	function TrPageSubscrProfile( $p=array() ){
		...
		$m 		= $p['m'];
			...
			$button = ($m['id']<>COMPANY_ID)? $this->ButtonSubscriptions(array('flag'=>$m['flag_subscriptions'],'company_id_out'=>$m['id'] )) : '';
		...
		$code = '	
				<div class="request-head">
					...
					<ul class="request-menu">
						...
						<li class="request-menu-item">
							<button>
								<a href="/chat/wt-chats" class="nav-link '.$views_wt_chats.'">Без темы</a>
							</button>
						</li>						
						<li class="request-menu-item">
							<button>
	...
	function TrPageMessagesFolders( $p=array() ){
		...
		$status_badge  = ($m["status"] != 2) ? 'badge-warning'            : 'badge-secondary';
			...
			if(!empty($m['avatar'])){ // если явно проставлен логотип чата
				...
				{				
					$avatar = !empty($rcm[0]['ava_company'])? 
						"<img src='".$rcm[0]['ava_company']."' class='rounded-circle' height='50'>":
						"<img src='/image/profile-icon.png' class='rounded-circle' height='50'>"; 
				}
			...
			if((!empty($m['folder_name'])) && ($m['owner_id'] === COMPANY_ID) && ($m['status'] != 2) ){ // если явно прописана тема чата и чат создан тем кому он пренадлежит и не в архиве	
				...
				{
					$theme = 'надо будет наладить';//$m["name_rcmc"].$b_comp;					
					$edit_theme = '';
				}
	...
	function TrPageMessages( $p=array() ){
		...
		$views 	= $p['views'];
		$m 		= $p['m'];		
		$tr =  $kol_notification = $cl_own = '';
	
		//$status_on_off = ($m["status"] != 2) ? 'В архив': 'Открыть тему';

			//$rcm = $this->rowChatMessages(array('company_id'=>$m['id']));
...
/* 			if (empty($m['ava_folder'] && $m['ava_company']))
				{ 
					$avatar = "<img src='/image/profile-icon.png' class='rounded-circle' height='40'>";
				} else {
					$avatar = "<img src='/image/profile-icon.png' class='rounded-circle' height='40'>";
				}	
					 */

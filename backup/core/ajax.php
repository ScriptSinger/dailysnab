...
$row=$r=array();
	...
	$in = array('email', 'pass', 'pass_again', 'id'=>'integer', 'value', 'flag', 'parent_id', 'categories', 'level', 
				...
				'attribute', 'active',
				'mclass', 'views',
				'categories_id', 'unit_id', 'unit_group_id', 'attribute_id', 'table' , 'limit_sell' , 'limit_buy', 
				'desc_sell', 'desc_buy', 'keywords_buy', 'keywords_sell','description_buy','description_sell',
				...
				'subject','messagetext','companies','potrbs','mid','fid','cid','url',
				);
...
/**/
			...
			if($rez){
				$arr = $f->GetCodeForm(array('id'=>$login_id,'phone_email_code'=>$in['phone_email_code']));	
				...
				if(!empty($r)){
					...
					if($login_id){
						...
						$STH = PreExecSQL(" INSERT INTO company (login_id, flag_account, legal_entity_id, company, iti_phone, phone, cities_id) VALUES (?,?,?,?,?,?,?); " ,
												array( $login_id,1,1,$company,'ru',$phone,$cities_id ));
						$company_id = $db->lastInsertId();
						if($STH){
								// Права и Роль пользователя на компанию(аккаунт)
								$STH = reqInsertLoginCompanyPrava(array(	'login_id'		=> $login_id,
...
elseif($_GET['route'] == 'save_password'){
	...
	if(!$flag){
			...
			if($login_id){
					...
					if($STH){
						$ok		= true;
						$g->AutorizeUser(array(	'login_id'=>$in['value'] ));//авторизация пользователя							
					}
			}
	}
...
elseif($_GET['route'] == 'modal_search'){
	...
	$arr = $t->ModalSearch(array('flag_buy_sell'=>$in['flag_buy_sell']));
...
elseif($_GET['route'] == 'save_buy_sell'){
	...
	if( !$categories_id && $in['status']==1 ){
		$categories_id = 821; // "Не существующая"
	}
	

	if(!COMPANY_ID){// если не авторизованный создает
		$in['status'] 	= ($in['status']==10)? 10 : 2;
		$in['status'] 	= ($categories_id==821)? 1 : $in['status'];
		$noautorize		= true;
	}
	...
	if($in['status']==10||$in['status']==12||$in['status']==14||$in['status']==15){// Предложение или Исполнено или Возврат или Возвращено
		...
		$r_p 	= reqBuySell(array('id' => $in['parent_id']));
		$copy_id 			= $r_p['copy_id'];
		if($in['status']==10){
			$url	= 'offer';
		...
		if(v_int($in['company_id'])){// существует ли компания
			$r 	= reqCompany(array( 'id' => $in['company_id'] ));
			if(!empty($r)){
	...
	if(!$in['id']){// СОЗДАНИЕ
			...
			if($flag_limit){
				...
				if($arr['STH']){
						...
						//						если номенклатура привязана, то обновляем в другом месте...
							if( $categories_id<>821 && !$noautorize && !$in['nomenclature_id'] && ($in['flag_buy_sell']==2) && ($in['status']==1||$in['status']==2||$in['status']==3) ){
							...
							}elseif($categories_id==821){// "Не существующая" категория , номенкталура не участвует
									$flag_nomenclature = true;
							}
						///
			...
			if($flag_limit){
					...
					$STH = PreExecSQL(" UPDATE buy_sell SET login_id=?, company_id2=?, categories_id=?, status_buy_sell_id=?, name=?, url=?, cities_id=?, urgency_id=?, currency_id=?, cost=?, form_payment_id=?, amount=?, comments=?, comments_company=?, responsible_id=?, availability=?, multiplicity=?, min_party=?, delivery_id=?, stock_id=?, assets_id=? ".$sql_data." WHERE id=?; " ,
											array( LOGIN_ID,$company_id2,$categories_id,$in['status'],$in['name'],$url,$in['cities_id'],$in['urgency_id'],$in['currency_id'],$in['cost'],$in['form_payment_id'],$in['amount'],$in['comments'],$in['comments_company'],$in['responsible_id'],$in['availability'],$in['multiplicity'],$in['min_party'],$in['delivery_id'],$in['stock_id'],$in['assets_id'],$in['id'] ));
...
elseif($_GET['route'] == 'autocomplete_search'){
	...
	$flag_buy_sell = $in['flag_buy_sell'];// по умолчанию
	...
	if($in['flag']=='modal'||$in['flag']=='top'||$in['flag']=='mainpage'){
			...
			$flag_buy_sell = $rf['flag_buy_sell'];
...
// autocomplete Сообщения
elseif($_GET['route'] == 'autocomplete_messages'){
	$rcm	= reqChatMessages(array('value' => $in['value'] , 'count_page'=>5));

	foreach($rcm as $i => $m){
		$jsd[] = array( 'id'=>$m['id'] , 	'name' 			=> $m['name_rcmc'],
											'name2' 		=> $m['ticket_exp'],
											'id' 			=> $m['id'],
											'value' 		=> $m['ticket_exp'],
											'message_id'	=> $m['folder_id'] );
	}
	if (empty($jsd)) $jsd[] = array('name' => '<span class="text-muted">не найдено</span>' , 'name2' =>'не найдено' );
}
// 
...
elseif($_GET['route'] == 'search_buy_sell'){
	...
	// Допустимые категории для запроса на стороннии ресурсы
		...
		if(isset($rc['parent_id'])){
			...
			if(in_array($rc['parent_id'], $at) && ($rf['flag_buy_sell']==2)){//
					$code_qrq = '';
...
elseif($_GET['route'] == 'save_search_filter_param_company'){
	...
	if($in['where']=='modal'){
		$flag_buy_sell = ($in['flag_buy_sell'])? 	$in['flag_buy_sell']		: 0;
		...
		$rf = reqSearchFilterParamCompany(array('login_id'=>LOGIN_ID,'company_id'=>COMPANY_ID));
		$rf = !empty($rf)? $rf : array( 'flag_buy_sell'=>0 , 'categories_id'=>0 , 'cities_id'=>0 );
		$flag_buy_sell = $rf['flag_buy_sell'];
		$categories_id	= ($in['categories_id'])? 	$in['categories_id'] : $rf['categories_id'];
	...
	$flag_interest 		= ($in['interests_id'])? 		1 				: 0;
		...
		// проверяем заполнено ли (cookie_session)
		if($rf['cookie_session']){
			...
			if($STH){		
					$STH = PreExecSQL(" INSERT INTO search_filter_param_company (login_id,company_id,cookie_session,flag_buy_sell,categories_id,cities_id,flag,flag_interest) VALUES (?,?,?,?,?,?,?,?); " ,
												array( LOGIN_ID,COMPANY_ID,COOKIE_SESSION,$flag_buy_sell,$categories_id,$cities_id,$in['flag'],$flag_interest ));
			}
...
elseif($_GET['route'] == 'modal_offer_buy'){
		...
		$row_bs = reqBuySell(array('id' => $in['id']));
...
elseif($_GET['route'] == 'save_buy_offer'){
	...
	$code = 'Нельзя купить';
			...
			$row_bs = reqBuySell(array('id'=>$in['buy_sell_id']));
		...
		if(!empty($row_bs)&&empty($error_qrq)&&empty($code_qrq)){
...
elseif($_GET['route'] == 'amo_basket'){
	...
	$ok = $flag_clear_parent = $parent_id = $errors_message = false;	
	...
	if($json){
		...
		if( $status ){
				...
				$code = $errors_message.' '.$warnings_message;
...
elseif($_GET['route'] == 'action_subscriptions'){
	...
	$ok = $STH = $flag_modal_amo_accounts = false;	
	$code = 'Нельзя выполнить';
	$button = $qrq_id = '';
	...
	if(empty($r)){
		$flag = true;
		$ok = ($r_k['kol']<10)? true : false;// не более 10 подписок
	}else{
		$flag = false;
	}
			...
			if($STH){
				// Оповещение
					$cn->StartNotification(array(	'flag'				=> 'subscriptions',
													...
													'tid'				=> COMPANY_ID,
													'notification_id'	=> 13 ));
				// Проверяем выводить авторизацию на сторонних ресурсах Amo
				$rc = reqCompany(array('id'=>$in['id']));
				$rq = reqSlovQrq(array('login_id'=>$rc['login_id']));
				if( $rq['id'] ){
					$flag_modal_amo_accounts 	= true;
					$qrq_id 					= $rq['id'];
				}
			}
		}else{
	...
	$jsd['button']	= $button;
	$jsd['count']	= $r_k['kol'];
	$jsd['flag_modal_amo_accounts']	= $flag_modal_amo_accounts;
	$jsd['qrq_id']					= $qrq_id;
}
...
elseif($_GET['route'] == 'save_assets_sell'){
	...
	$row_bs 	= reqBuySell(array('id'=>$in['buy_sell_id'],'company_id'=>COMPANY_ID));
...
// модальное окно Товары на складе закрепленные за номенклатурой
...
elseif($_GET['route'] == 'save_accountsadd'){
	...
	if($arr['id']){
		...
		if(!$in['id']){// СОЗДАНИЕ
				$STH = PreExecSQL(" INSERT INTO amo_accounts (company_id, vendorid, login, password, accounts_id, accounts_title) VALUES (?,?,?,?,?,?); " ,
										array( COMPANY_ID,$in['value'],$in['login'],$in['pass'],$arr['id'],$arr['title'] ));
				if($STH){
					$id = $db->lastInsertId();
					$code 	= 'Аккаунт поставщика добавлен';
				}
		}else{// РЕДАКТИРОВАНИЕ
				$STH = PreExecSQL(" UPDATE amo_accounts SET login=?, password=?, accounts_id=?, accounts_title=?, data_update=NOW() WHERE id=? AND company_id=?; " ,
										array( $in['login'],$in['pass'],$arr['id'],$arr['title'],$in['id'],COMPANY_ID ));
				if($STH){
					$id = $in['id'];
...
<body>
	...
	<table class="list">
		<thead>
			<tr>
				<th width="5%">№</th>
				<th width="54%">Наименование услуг</th>
...
elseif($_GET['route'] == 'modal_amo_accounts'){
	
	// Проверяем есть ли поставщик в AMO (добавляли ранее)
	$rq 	= reqSlovQrq(array('id'=>$in['id']));
	$ra 	= reqAmoAccounts(array( 'vendorid'=>$rq['vendorid'] , 'company_id'=>COMPANY_ID , 'flag'=>'one'));
	
	if(empty($ra)){// ранее не добавлен
	
		$arr = $f->FormAmoAccount(array('row'=>$rq));
		//vecho($arr);
		$code = $t->getModal(	array('class_dialog'=>'search-dialog needs-dialog','content'=>$arr['content']),
...
elseif($_GET['route'] == 'reply_message'){
		...
		if(!empty($in['messagetext'])) {
			...
			if(!empty($mid)){								
				...
				//Добавление информации о сообщении
				$STH2 = PreExecSQL(" INSERT INTO tickets (folder_id,company_id,companies,ticket_exp,ticket_status,attachments) VALUES (?,?,?,?,?,?); " ,
								array($mid,COMPANY_ID,$companies_id,$messagetext,$ticket_status,$attachments));
				$message_id = $db->lastInsertId();	
...
elseif($_GET['route'] == 'update_theme_info'){
		...
		$folder_status 	= 1; //1 - открытый, 2 - архивный, 3 - приватный (только для 2-х собеседников) 
		$avatar = '';
		//$comp_ids_input = COMPANY_ID.','.$in['companies'];
		...
		//$companies_json = json_encode(); //преобразуем для загрузки в БД 
		$companies_json = json_encode(explode(',',implode(",",$comp_ids_input)));
		//var_dump($companies_json);
		//var_dump($comp_ids);	
		...
		if(!empty($comp_ids_input)){ //проврека на наличие собеседников, хотя один собеседник должен быть
				...
				if(count($comp_ids_input)>1 && empty($in['subject'])){
					...
					if (count($comp_ids_input)==1){ //если один собеседник, то тема не обязательна
						...
						$STH = PreExecSQL(" UPDATE tickets_folder SET folder_name=?,status=?,companies_id=?,avatar=? WHERE id=?" ,
													array($folder_name,$folder_status,$companies_json,$avatar,$folder_id));						
						if($STH){
							$ok = true;
							$code = 'Сохранено';									
...
elseif($_GET['route'] == 'save_admin_qrq'){
	...
	if(!$in['id']){// СОЗДАНИЕ
		...
		if($STH){
			...
			$STH = PreExecSQL(" INSERT INTO slov_qrq (qrq,login_id,company_id,vendorid) VALUES (?,?,?,?); " ,
									array( $in['name'],$login_id,$company_id,$in['value'] ));
